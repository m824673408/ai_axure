...
describe('GitHub governance', () => {
  ...
  it('setup 生成固定 CODEOWNERS 与 Prototype Gate，status 和 lint 可识别弱化', () => {
    ...
    expect(status.toolRef).toBe('v0.2.0-rc.5');
    ...
    expect(workflow).toContain('PROTOTYPE_TOOL_REF: v0.2.0-rc.5');
    expect(workflow).toContain('git clone --depth 1 --branch "$PROTOTYPE_TOOL_REF"');
    expect(workflow).toContain('npm ci --prefix "$RUNNER_TEMP/ai_axure" --include=dev --ignore-scripts');
    expect(workflow).toContain('cd "$RUNNER_TEMP/ai_axure"');
    expect(workflow).toContain('npm pack --ignore-scripts --pack-destination "$RUNNER_TEMP" --silent');
    expect(workflow).toContain('npm install --global "$RUNNER_TEMP"/product-prototype-core-*.tgz --ignore-scripts');
  ...
  it('CLI governance status --json 提供稳定机器输出', () => {
    ...
    expect(body).toMatchObject({ status: 'READY', enabled: true, owner: 'm824673408', toolRef: 'v0.2.0-rc.5' });
  });
});
